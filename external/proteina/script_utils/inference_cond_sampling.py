...
class GenDataset(Dataset):
    ...
    def __init__(
        ...
        if nres is not None and cath_codes is not None:
            self.nres = []
            self.cath_codes = []
            self.nsamples = []
            # Use enumerate to get the index 'i'
            for i, length in enumerate(nres):
                self.nres.append(length)
                # Use nsamples[i] to get the integer count for this specific length
                self.cath_codes.append([[cath_codes[0]]] * nsamples[i]) 
                self.nsamples.append(nsamples[i])
...
if __name__ == "__main__":
    ...
    model = Proteina.load_from_checkpoint(ckpt_file).float()
    ...
    dataset = GenDataset(
        nres=cfg.nres_lens,  # This pulls [95, 100, 105, 110, 115] from your yaml
        cath_codes=args.cath_codes, 
        nsamples=args.nsamples, 
        dt=cfg.dt, 
        len_cath_codes=len_cath_codes
    )
